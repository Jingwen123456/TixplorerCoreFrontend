    ...
    public class MembersController : Controller
    {
        ...
        public async Task<IActionResult> myFavorite()
        {
            ...
            if (mData != null)//若有找到資料
            {
                ...
                //宣告一個新的pData陣列以儲存查詢結果(儲存型態為object)
                object[] pData = new object[arrFavorite.Length];

                //查詢商品的資料，將每筆資料以myFavoriteVM的型別儲存在pData裡
                for (int i=0;i< arrFavorite.Length; i++)
                {
                    pData[i] = _context.Products
                   .Where(p => p.PId == arrFavorite[i])
                   .Select(p => new myFavoriteVM
                   {
                ...
                //將查詢完的各個資料存在ViewData["productDatas"]
                ViewData["productDatas"] = pData;
        ...
        [HttpGet]
        public async Task<IActionResult> removeFromFavorite(string Pid,string Account)
        {
            var mData = _context.Members.FirstOrDefault(d => d.Email == Account);

                mData.Favorite = mData.Favorite.Replace($"{Pid};", "");
                _context.SaveChanges();
