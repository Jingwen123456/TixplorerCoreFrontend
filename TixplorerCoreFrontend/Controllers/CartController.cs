...
namespace TixplorerFore.Controllers
{
    public class CartController : Controller
    {
        ...
        // 修改購物車商品數量
       [HttpPost]
        public ActionResult Edit(string id, int count)
        {
            if (HttpContext.Session.GetString(CDictionary.TIXPLORER_USER_DATA) != "" && HttpContext.Session.GetString(CDictionary.TIXPLORER_USER_DATA) != null)
            {
                ...
                {
                    TixplorerContext db = new TixplorerContext();
                    var editCartItem = db.Carts.FirstOrDefault(c => c.CId == id);

                    if (editCartItem != null && editCartItem.Type == 0)
                    {
                        // 計算該商品的平均單價
                        decimal itemPrice = editCartItem.Totalprice / editCartItem.Count;
                        // 修改項目的數量
                        editCartItem.Count = count;
                        editCartItem.Totalprice = count * itemPrice;
